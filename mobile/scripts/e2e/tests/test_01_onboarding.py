...
def run_test(driver: AndroidDeviceDriver, reporter: E2EReporter) -> bool:
    ...
    lang_card = driver.wait_for_element(text="English", desc="Select English language", timeout=8.0)
    if not lang_card:
    ...
    t0 = time.time()
    hindi_card = driver.find_element(text="हिंदी", desc="हिंदी भाषा चुनें")
    if hindi_card:
    ...
    t0 = time.time()
    driver.tap(lang_card[0], lang_card[1], sleep_after=0.5)

    # 5. Verify Auth screen is reached
    auth_header = driver.wait_for_element(text=["Continue with Google", "Log In", "Login"], timeout=6.0)
