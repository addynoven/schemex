...
def run_test(driver: AndroidDeviceDriver, reporter: E2EReporter) -> bool:
    ...
    has_vault = (
        driver.wait_for_element(text="Upload once, reuse", timeout=5.0)
        or driver.wait_for_element(text="QUICK ACTIONS", timeout=5.0)
        or driver.find_element(text="Vault")
    )
    ...
    if readiness_btn:
        driver.tap(readiness_btn[0], readiness_btn[1], sleep_after=1.5)

    sc_meter = driver.capture_screenshot("14_vault_readiness_meter")
    has_meter = (
        driver.wait_for_element(text="Select a scheme to check readiness", timeout=4.0)
        or driver.wait_for_element(text="documents present", timeout=4.0)
        or driver.find_element(text="REQUIRED DOCUMENTS")
    )
    ...
    if has_meter:
        ...
        driver.press_key(111)  # ESCAPE / BACK
        time.sleep(0.5)
        # Also press back icon if still on readiness
        back_btn = driver.find_element(text="\uf053")
        if back_btn:
            driver.tap(back_btn[0], back_btn[1], sleep_after=1.0)
    else:
